...
function renderTicketCard(ev) {
    ...
    const footer = ev.status === 'past'
        ...
        : `<span class="ticket-price">${formatINR_small(ev.normalPrice)} onwards</span>
           <a href="#" class="ticket-btn ${sm.cardClass.replace('-card', '-btn')} booking-trigger"
              data-normal="${ev.normalPrice || 0}"
              data-premium="${ev.premiumPrice || 0}"
              data-luxury="${ev.luxuryPrice || 0}">Get Tickets →</a>`;
...
document.getElementById('bookingSubmitBtn').addEventListener('click', async () => {
    ...
    const ok = await saveDoc('ch_tickets', {
        ...
        ticketType: selectedType ? selectedType.dataset.type : 'normal',
